...
double knapsack(Ordine *ordini, Veicolo *veicolo, int *contatoreDegliOrdini)
{
    ...
    int volumeMassimo = veicolo->volume;
    double matrice[*contatoreDegliOrdini + 1][pesoMassimo + 1][volumeMassimo + 1];

    for (int indiceDellOrdine = 0; indiceDellOrdine <= *contatoreDegliOrdini; indiceDellOrdine++)
    {
        for (int peso = 0; peso <= pesoMassimo; peso++)
        {
            for (int volume = 0; volume <= volumeMassimo; volume++)
            {
                if (indiceDellOrdine == 0 || peso == 0 || volume == 0)
                    matrice[indiceDellOrdine][peso][volume] = 0;
                else if (ordini[indiceDellOrdine - 1].peso <= peso && ordini[indiceDellOrdine - 1].volume <= volume)
                {
                    int include = ordini[i - 1] + dp[i - 1][w - wt[i - 1]][v - vol[i - 1]];
                    int pesoDellOrdini = ordini[indiceDellOrdine - 1].peso;
                    matrice[indiceDellOrdine][peso][volume] = piuGrande(ordini[indiceDellOrdine - 1].valore + matrice[indiceDellOrdine - 1][peso - pesoDellOrdini], matrice[indiceDellOrdine - 1][peso]);
                }
                else
                    matrice[indiceDellOrdine][peso][volume] = matrice[indiceDellOrdine - 1][peso][volume];
            }
        }
    }
